-- Fix "syntax error at or near array": in dynamic SQL inside $q$...$q$, use 'array' and '^\d+$'
-- (single quotes) not ''array'' so the executed SQL has valid string literals.
...
COMMENT ON FUNCTION get_stats_overview(text, text) IS 'Overview stats. Ban: count each ban (both teams ban same = 2). String literals in dynamic SQL use single quotes.';
