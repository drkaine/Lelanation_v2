-- Matchups / counters: uniquement contre l'adversaire dans la même lane (même rôle).
-- Avant: tout adversaire de l'équipe opposée. Après: opp.role = p.role (même lane).
...
COMMENT ON FUNCTION get_matchups_by_champion(int, text, text, int) IS 'Winrate vs each opponent in the same lane (same role). Optional version and rank_tier filter.';
