-- Parties par division basées sur participants.rank_tier (aligné avec l'onglet Champions).
-- Un match est compté dans une division si au moins un participant a ce rank_tier.
...
BEGIN
  ...
  -- Divisions depuis participants.rank_tier (aligné avec filtre Champions par rang)
  EXECUTE format(
    $q$
    SELECT jsonb_agg(
    ...
    FROM (VALUES ('IRON',1),('BRONZE',2),('SILVER',3),('GOLD',4),('PLATINUM',5),('EMERALD',6),('DIAMOND',7),('MASTER',8),('GRANDMASTER',9),('CHALLENGER',10),('UNRANKED',11)) AS t(rank_tier, ord)
    LEFT JOIN (
      SELECT UPPER(TRIM(p.rank_tier)) AS rank_tier, COUNT(DISTINCT p.match_id)::bigint AS cnt
      FROM participants p
      INNER JOIN matches m ON m.id = p.match_id
      WHERE %s AND p.rank_tier IS NOT NULL AND p.rank_tier != ''
      GROUP BY UPPER(TRIM(p.rank_tier))
...
COMMENT ON FUNCTION get_stats_overview(text) IS 'Overview stats; optional p_version. matchesByDivision uses participants.rank_tier (aligné avec filtre Champions)';
