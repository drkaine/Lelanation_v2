-- Botlane duo-vs-duo aggregate MV: mv_botlane_duo_vs_duo_stats
-- Purpose: compute 2v2 botlane matchup stats, ex. (Ezreal+Thresh) vs (Lucian+Nami).
--
-- Each row is directional: (adc_id, support_id) against (opp_adc_id, opp_support_id)
-- with filters carried by rank_tier / rank_division / game_version / region.
...
WITH botlane_duos AS (
  -- One row per match+team where the team has exactly one ADC and one SUPPORT (as per match_players.role)
  SELECT
    m.id AS match_id,
    m.region,
    m.game_version,
  ...
  GROUP BY m.id, m.region, m.game_version, m.rank_tier, m.rank_division, t.id, t.win
  HAVING
    COUNT(*) FILTER (WHERE mp.role = 'ADC') = 1
    AND COUNT(*) FILTER (WHERE mp.role = 'SUPPORT') = 1
),
duo_vs_duo AS (
  -- Join each team botlane duo with the opponent team botlane duo for the same match.
  SELECT
    a.region,
    a.game_version,
    a.rank_tier,
...
CREATE UNIQUE INDEX ON mv_botlane_duo_vs_duo_stats (
  adc_id, support_id, opp_adc_id, opp_support_id, rank_tier, rank_division, game_version, region
);
