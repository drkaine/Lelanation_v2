-- Add synergy/together aggregate MV with ally role: mv_champion_duo_role_stats
-- Enables lane-duo analysis (ex. ADC + SUPPORT) by filtering on ally_role.
--
-- Definition:
...
-- - ally_role: role of champion B (TOP/JUNGLE/MID/ADC/SUPPORT)
-- - count_game/count_win: games played together and wins for champion A when paired with B in that ally role
--
-- Notes:
-- - Role filter applies to champion A (mp.role). Ally role is stored separately as ally_role.
...
-- Extend close_patch() archiving to include mv_champion_duo_role_stats.
...
BEGIN
  ...
  ELSE
    ...
    EXECUTE format(
      'CREATE TABLE IF NOT EXISTS archive_champion_duo_role_%s AS SELECT * FROM mv_champion_duo_role_stats WHERE game_version = %L OR game_version LIKE %L',
      v_table_suffix, p_game_version, v_like_patch
    );
    EXECUTE format(
