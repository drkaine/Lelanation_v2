-- Parties par division : participant.rank_tier en priorité, match.rank en secours si tous les participants ont rank_tier null.
-- Évite de sous-compter quand beaucoup de participants n'ont pas encore été backfillés.
...
BEGIN
  ...
  -- Divisions : participant.rank_tier OU match.rank si aucun participant n'a de rang (fallback)
  EXECUTE format(
    $q$
    SELECT jsonb_agg(
    ...
    FROM (VALUES ('IRON',1),('BRONZE',2),('SILVER',3),('GOLD',4),('PLATINUM',5),('EMERALD',6),('DIAMOND',7),('MASTER',8),('GRANDMASTER',9),('CHALLENGER',10),('UNRANKED',11)) AS t(rank_tier, ord)
    LEFT JOIN (
      SELECT rank_tier, COUNT(DISTINCT match_id)::bigint AS cnt
      FROM (
        SELECT p.match_id, UPPER(TRIM(p.rank_tier)) AS rank_tier
        FROM participants p INNER JOIN matches m ON m.id = p.match_id
        WHERE %s AND p.rank_tier IS NOT NULL AND p.rank_tier != ''
        UNION
        SELECT m.id AS match_id, UPPER(TRIM(split_part(m.rank, '_', 1))) AS rank_tier
        FROM matches m
        WHERE %s AND m.rank IS NOT NULL AND m.rank != ''
        AND NOT EXISTS (SELECT 1 FROM participants p2 WHERE p2.match_id = m.id AND p2.rank_tier IS NOT NULL AND p2.rank_tier != '')
      ) sub
      GROUP BY rank_tier
...
COMMENT ON FUNCTION get_stats_overview(text) IS 'Overview stats; matchesByDivision uses participant.rank_tier, fallback to match.rank when all participants have null rank';
