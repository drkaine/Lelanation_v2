-- Fix "syntax error at or near "["": avoid literal "[" in dynamic SQL by using escape form for empty jsonb array.
-- E'\x5b\x5d' = '[]' so COALESCE(..., E'\x5b\x5d'::jsonb) is equivalent to COALESCE(..., '[]'::jsonb).
...
DECLARE
  ...
  key_bans text := 'bans';
  empty_jsonb_lit text := 'E''\x5b\x5d''::jsonb';
BEGIN
...
COMMENT ON FUNCTION get_stats_overview(text, text) IS 'Overview stats. Ban: count each ban (both teams ban same = 2). Empty jsonb in dynamic SQL via %s to avoid "[" syntax error.';
