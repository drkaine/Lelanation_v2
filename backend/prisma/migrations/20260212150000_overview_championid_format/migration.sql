-- Fix get_stats_overview: inject JSON key via format(%L, 'championId') so the executed SQL
-- has a proper string literal; ''championId'' in $q$ is parsed as quote+identifier+quote and causes 42601.
...
BEGIN
  ...
  /* Key injected via %L so executed SQL has proper string literal 'championId' */
  EXECUTE format(
    $q$
    SELECT COALESCE(
      jsonb_agg(
...
COMMENT ON FUNCTION get_stats_overview(text, text) IS 'Overview stats; JSON keys injected via format(%L) to avoid syntax error.';
