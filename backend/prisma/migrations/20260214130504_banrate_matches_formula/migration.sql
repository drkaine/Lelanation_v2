-- Banrate overview: align with formula banrate(X) = |{ m in M : X ban in m }| / |M|
-- (number of matches where champion is banned / total matches), same as get_stats_champions.
-- Before: ban_count = total ban slots (both teams = 2), banrate = ban_count/(2*|M|).
...
BEGIN
  ...
  /* banrate = |{ m in M : X ban in m }| / |M| (distinct matches where champion is banned). */
  EXECUTE format(
    $q$
    SELECT COALESCE(
      jsonb_agg(
...
COMMENT ON FUNCTION get_stats_overview(text, text) IS 'Overview stats. Banrate = |matches where champion banned| / |M| (same as get_stats_champions).';
