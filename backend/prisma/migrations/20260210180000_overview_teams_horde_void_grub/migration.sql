-- Add horde (Void Grub) objective; Riot API may send objectives.horde. Keep riftHerald for backward compat.
...
COMMENT ON FUNCTION get_stats_overview_teams(text) IS 'Stats from matches.teams: bans, objectives (first + kills + distribution). Includes horde (Void Grub).';
