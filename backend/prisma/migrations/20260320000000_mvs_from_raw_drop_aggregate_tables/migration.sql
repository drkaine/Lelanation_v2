-- Phase 2: MVs champion + satellites from raw (core_stat_id). Puis DROP des tables d'agrégat.
-- Une seule source de vérité : données brutes + active_patches ; MVs = agrégats directs.

-- 1. Drop MVs dépendant des tables d'agrégat (satellites puis core/vs)
...
-- 2. mv_champion_core_stats depuis brutes (match_players + match_player_core + matchs + teams + bans)
CREATE MATERIALIZED VIEW mv_champion_core_stats AS
WITH base AS (
  SELECT
    mp.champion_id,
    COALESCE(NULLIF(trim(mp.rank_tier), ''), m.rank_tier) AS rank_tier,
    COALESCE(mp.rank_division, m.rank_division, '') AS rank_division,
    m.game_version,
    mp.role,
    m.region,
    t.win,
...
ban_counts AS (
  ...
  WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
  GROUP BY b.champion_id, m.rank_tier, m.rank_division, m.game_version, m.region
)
SELECT
  core_stat_id(b.champion_id, b.rank_tier, b.rank_division, b.game_version, b.role, b.region) AS id,
  b.champion_id,
  b.rank_tier,
  b.rank_division,
  b.game_version,
...
LEFT JOIN ban_counts bc ON bc.champion_id = b.champion_id
  AND bc.rank_tier = b.rank_tier AND bc.rank_division = b.rank_division
...
-- 3. mv_champion_vs_stats depuis brutes (paires même match, équipe opposée, même rôle)
CREATE MATERIALIZED VIEW mv_champion_vs_stats AS
SELECT
  core_stat_id(mp.champion_id, COALESCE(NULLIF(trim(mp.rank_tier), ''), m.rank_tier), COALESCE(mp.rank_division, ''), m.game_version, mp.role, m.region) AS champion_stat_id,
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, opp.champion_id;

CREATE UNIQUE INDEX ON mv_champion_vs_stats (champion_stat_id, opponent_champion_id);

-- 4. Satellites depuis brutes (champion_core_stat_id = core_stat_id(...))
CREATE MATERIALIZED VIEW mv_champion_first_objectif_stats AS
SELECT
  core_stat_id(mp.champion_id, COALESCE(NULLIF(trim(mp.rank_tier), ''), m.rank_tier), COALESCE(mp.rank_division, ''), m.game_version, mp.role, m.region) AS champion_core_stat_id,
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division;
...
-- Runes/shard/item/summoner/spell/bucket: aggregate from raw with grouping key
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, s.shard_id, s.slot;
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, r.perk_id, r.style;
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, COALESCE(sl.shard_list, '');
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, COALESCE(rl.rune_list, '');
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, i.item_id;
...
WITH item_lists AS (
  SELECT match_player_id,
    ('[' || string_agg(item_id::text, ',' ORDER BY "order") || ']') AS item_list
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, COALESCE(il.item_list, '[]');
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, so.spell_slot;
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, ss.spell_id;
...
WHERE (split_part(m.game_version, '.', 1) || '.' || split_part(m.game_version, '.', 2)) IN (SELECT game_version FROM active_patches)
GROUP BY mp.champion_id, mp.rank_tier, mp.rank_division, m.game_version, mp.role, m.region, m.rank_tier, m.rank_division, b.duration_bucket;

CREATE UNIQUE INDEX ON mv_champion_bucket (champion_stat_id, duration_bucket);

-- 5. DROP aggregate tables (order: satellites first, then vs, core, team)
DROP TABLE IF EXISTS champion_bucket;
DROP TABLE IF EXISTS champion_summoner_spells;
DROP TABLE IF EXISTS champion_spell_solo_stats;
DROP TABLE IF EXISTS champion_item_stats;
DROP TABLE IF EXISTS champion_item_solo_stats;
DROP TABLE IF EXISTS champion_runes_stats;
DROP TABLE IF EXISTS champion_shard_stats;
DROP TABLE IF EXISTS champion_runes_solo_stats;
DROP TABLE IF EXISTS champion_shard_solo_stats;
DROP TABLE IF EXISTS champion_challenge_stats;
DROP TABLE IF EXISTS champion_matchup_stats;
DROP TABLE IF EXISTS champion_combat_stats;
DROP TABLE IF EXISTS champion_vision_stats;
DROP TABLE IF EXISTS champion_objectif_stats;
DROP TABLE IF EXISTS champion_first_objectif_stats;
DROP TABLE IF EXISTS champion_vs_stats;
DROP TABLE IF EXISTS champion_core_stats;
DROP TABLE IF EXISTS team_core_stats;

-- 6. Bootstrap active_patches from matchs (if empty)
INSERT INTO active_patches (game_version)
SELECT DISTINCT (split_part(game_version, '.', 1) || '.' || split_part(game_version, '.', 2)) AS gv
FROM matchs
WHERE game_version IS NOT NULL AND game_version != ''
ON CONFLICT (game_version) DO NOTHING;

-- 7. refresh_all_materialized_views: include all MVs (champion core/vs + satellites + team)
