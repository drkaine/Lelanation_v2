-- Runes overview: support both runes = array of styles and runes = { styles: [] }; keep winrate at 2 decimals.
-- Winrate = SUM(win)/COUNT(*) per rune (participant-level, already correct); ensures ROUND(..., 2).
...
BEGIN
  ...
  rune_flat AS (
    SELECT (sel->>'perk')::int AS perk_id, ra.win
    FROM runes_array ra,
...
COMMENT ON FUNCTION get_stats_overview_detail(text, text) IS 'Overview detail (runes, items, spells). Runes: support runes or runes.styles; winrate per participant per rune.';
