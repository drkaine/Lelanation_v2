-- Progression perf: restrict p_team to participants in relevant matches only (avoids full table scan).
-- Indexes to speed up relevant_matches and p_team.

CREATE INDEX IF NOT EXISTS idx_matches_game_version_progression
  ON matches (game_version)
  WHERE game_version IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_participants_match_id_progression
  ON participants (match_id);

-- 1) get_stats_overview_progression
