-- Fix "syntax error at or near "["": avoid literal '[]' in EXECUTE format() by using
-- (E'\x5b\x5d')::jsonb instead of ''[]''::jsonb so the bracket is not in the SQL source.
...
COMMENT ON FUNCTION get_stats_overview(text, text) IS 'Overview stats. Empty jsonb in dynamic SQL via %s to avoid syntax error at "["';
