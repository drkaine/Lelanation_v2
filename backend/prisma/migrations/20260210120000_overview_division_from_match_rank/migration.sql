-- Parties par division : basées uniquement sur match.rank (tier = partie avant '_', ex. GOLD_II -> GOLD).
-- Aligné avec la donnée rang stockée au niveau match.
...
BEGIN
  ...
  -- Divisions : uniquement match.rank (tier = partie avant '_'; null/vide -> UNRANKED)
  EXECUTE format(
    $q$
    SELECT jsonb_agg(
    ...
    FROM (VALUES ('IRON',1),('BRONZE',2),('SILVER',3),('GOLD',4),('PLATINUM',5),('EMERALD',6),('DIAMOND',7),('MASTER',8),('GRANDMASTER',9),('CHALLENGER',10),('UNRANKED',11)) AS t(rank_tier, ord)
    LEFT JOIN (
      SELECT rank_tier, COUNT(*)::bigint AS cnt
      FROM (
        SELECT
          CASE
            WHEN m.rank IS NOT NULL AND TRIM(m.rank) != '' THEN UPPER(TRIM(split_part(m.rank, '_', 1)))
            ELSE 'UNRANKED'
          END AS rank_tier
        FROM matches m
        WHERE %s
...
COMMENT ON FUNCTION get_stats_overview(text) IS 'Overview stats; matchesByDivision uses match.rank (tier part only; null/empty -> UNRANKED)';
