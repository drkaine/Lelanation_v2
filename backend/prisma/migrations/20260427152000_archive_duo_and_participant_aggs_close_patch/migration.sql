...
BEGIN
  ...
  CREATE TABLE IF NOT EXISTS archive_agg_champion_duo_stats (LIKE agg_champion_duo_stats INCLUDING ALL);
  ...
  CREATE INDEX IF NOT EXISTS idx_archive_cdss_csid ON archive_agg_champion_duo_stats (champion_stat_id);
...
BEGIN
  ...
  INSERT INTO archive_agg_champion_duo_stats
  SELECT s.* FROM agg_champion_duo_stats s
  INNER JOIN agg_champion_core_stats c ON c.id = s.champion_stat_id
  WHERE c.game_version = p_game_version OR c.game_version LIKE v_like_patch
  ON CONFLICT DO NOTHING;
  GET DIAGNOSTICS v_n = ROW_COUNT;
  v_counts := v_counts || jsonb_build_object('agg_champion_duo_stats', v_n);
  ...
  DELETE FROM agg_champion_duo_stats
  WHERE champion_stat_id IN (
    SELECT id FROM agg_champion_core_stats WHERE game_version = p_game_version OR game_version LIKE v_like_patch
  );
