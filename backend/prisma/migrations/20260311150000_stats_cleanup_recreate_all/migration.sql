-- =============================================================================
-- Stats cleanup & full recreate: drop all views, materialized views, and
-- get_stats_* functions; recreate them for current DB (win from match_teams via p_team).
-- =============================================================================

-- ─── 1. DROP (order: MVs that depend on functions, then views, then MV detail, then functions, then helper views) ───

DROP MATERIALIZED VIEW IF EXISTS mv_stats_champions CASCADE;
...
DROP FUNCTION IF EXISTS get_stats_overview_detail(text, text);
DROP FUNCTION IF EXISTS get_stats_overview(text, text);
DROP FUNCTION IF EXISTS get_stats_overview_teams(text, text);
DROP FUNCTION IF EXISTS get_stats_champions(text, text);
DROP FUNCTION IF EXISTS get_stats_overview_duration_winrate(text, text);
DROP FUNCTION IF EXISTS get_stats_duration_winrate_by_champion(int, text, text);
DROP FUNCTION IF EXISTS get_stats_overview_progression(text, text);
DROP FUNCTION IF EXISTS get_stats_overview_progression_full(text, text);
...
BEGIN
  ...
  IF total_matches = 0 THEN
    ...
    RETURN jsonb_build_object(
      ...
      'matchesByVersion', COALESCE(by_version, '[]'::jsonb), 'topWinrateChampions', '[]'::jsonb, 'topPickrateChampions', '[]'::jsonb, 'topBanrateChampions', '[]'::jsonb
    );
  END IF;

  EXECUTE format('SELECT COUNT(DISTINCT p.player_id) FROM participants p INNER JOIN matches m ON m.id = p.match_id WHERE %s', match_cond) INTO player_count;

  EXECUTE format(
    $q$ SELECT jsonb_agg(jsonb_build_object('rankTier', t.rank_tier, 'matchCount', COALESCE((d.cnt)::int, 0)) ORDER BY t.ord)
    ...
    LEFT JOIN (SELECT UPPER(TRIM(split_part(rank, '_', 1))) AS rank_tier, COUNT(*)::bigint AS cnt FROM matches m WHERE %s AND rank IS NOT NULL AND rank != '' GROUP BY split_part(rank, '_', 1)) d ON d.rank_tier = t.rank_tier $q$,
    division_cond
  ) INTO by_division;
  ...
  EXECUTE format(
    ...
    FROM (
      SELECT p.champion_id, COUNT(*)::bigint AS games, SUM(CASE WHEN mt.win THEN 1 ELSE 0 END)::bigint AS wins,
      ...
      INNER JOIN matches m ON m.id = p.match_id WHERE %s
      GROUP BY p.champion_id HAVING COUNT(*) >= 20 ORDER BY winrate DESC LIMIT 10
    ) top $q$,
    key_champion_id, empty_arr, match_cond, match_cond
  ) INTO top_champs;

  EXECUTE format(
    $q$ SELECT COALESCE(jsonb_agg(jsonb_build_object(%L, champion_id, 'games', (games)::int, 'wins', (wins)::int, 'winrate', winrate, 'pickrate', pickrate) ORDER BY pickrate DESC), %s)
    FROM (
      SELECT p.champion_id, COUNT(*)::bigint AS games, SUM(CASE WHEN mt.win THEN 1 ELSE 0 END)::bigint AS wins,
        ROUND(100.0 * SUM(CASE WHEN mt.win THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0), 2) AS winrate,
        ROUND(100.0 * COUNT(*) / NULLIF((SELECT COUNT(*) FROM matches m WHERE %s), 0), 2) AS pickrate
      ...
      INNER JOIN match_teams mt ON mt.match_id = p.match_id AND mt.team_id = p_team.team_id
      INNER JOIN matches m ON m.id = p.match_id WHERE %s
      GROUP BY p.champion_id HAVING COUNT(*) >= 20 ORDER BY pickrate DESC LIMIT 5
    ) top $q$,
    key_champion_id, empty_arr, match_cond, match_cond
  ) INTO top_pickrate;

  EXECUTE format(
    $q$ SELECT COALESCE(jsonb_agg(jsonb_build_object('championId', champion_id, 'banCount', (ban_count)::int, 'banrate', banrate) ORDER BY banrate DESC), %s)
    FROM (
      SELECT sub.champion_id, sub.ban_count, ROUND(100.0 * sub.ban_count / NULLIF((SELECT COUNT(*) FROM matches m WHERE %s), 0), 2) AS banrate
      FROM (SELECT b.champion_id, COUNT(DISTINCT mt.match_id)::bigint AS ban_count FROM match_teams mt INNER JOIN matches m ON m.id = mt.match_id INNER JOIN bans b ON b.match_team_id = mt.id WHERE %s GROUP BY b.champion_id) sub
      ORDER BY banrate DESC LIMIT 5
    ) top $q$,
    empty_arr, match_cond, match_cond
  ) INTO top_banrate;

  RETURN jsonb_build_object(
    'totalMatches', (total_matches)::int, 'lastUpdate', to_jsonb(last_update), 'playerCount', (player_count)::int,
    'matchesByDivision', COALESCE(by_division, '[]'::jsonb), 'matchesByVersion', by_version,
    'topWinrateChampions', COALESCE(top_champs, '[]'::jsonb), 'topPickrateChampions', COALESCE(top_pickrate, '[]'::jsonb), 'topBanrateChampions', COALESCE(top_banrate, '[]'::jsonb)
  );
END;
$$;

-- ─── 6. RECREATE: get_stats_champions (win from match_teams) ───
...
BEGIN
  WITH filtered AS (
    SELECT p.champion_id, p.match_id, mt.win, COALESCE(p.role, 'UNKNOWN') AS role
    FROM participants p
    INNER JOIN (SELECT id, match_id, CASE WHEN ROW_NUMBER() OVER (PARTITION BY match_id ORDER BY id) <= 5 THEN 100 ELSE 200 END AS team_id FROM participants) p_team ON p_team.id = p.id AND p_team.match_id = p.match_id
    INNER JOIN match_teams mt ON mt.match_id = p.match_id AND mt.team_id = p_team.team_id
  ...
  champ_agg AS (SELECT champion_id, COUNT(*) AS games, SUM(win::int) AS wins FROM filtered GROUP BY champion_id),
  role_agg AS (SELECT champion_id, role, COUNT(*) AS games, SUM(win::int) AS wins FROM filtered GROUP BY champion_id, role),
  role_denom AS (SELECT role, COUNT(DISTINCT match_id)::bigint AS match_count FROM filtered GROUP BY role),
  ban_agg AS (
    SELECT b.champion_id, COUNT(*)::bigint AS ban_count
    FROM match_teams mt INNER JOIN bans b ON b.match_team_id = mt.id
    WHERE mt.match_id IN (SELECT match_id FROM filtered)
    GROUP BY b.champion_id
  ),
  presence_denom AS (SELECT (SELECT COUNT(DISTINCT match_id) FROM filtered) AS total),
  champ_with_roles AS (
    SELECT c.champion_id, c.games, c.wins,
      ...
      (SELECT jsonb_object_agg(r.role, jsonb_build_object('games', r.games, 'wins', r.wins, 'winrate', ROUND(100.0 * r.wins / NULLIF(r.games, 0), 2), 'pickrate', ROUND(100.0 * r.games / NULLIF(d.match_count, 0), 2)))
        FROM role_agg r LEFT JOIN role_denom d ON d.role = r.role WHERE r.champion_id = c.champion_id) AS byRole
    FROM champ_agg c LEFT JOIN ban_agg ba ON ba.champion_id = c.champion_id
  )
  SELECT jsonb_agg(jsonb_build_object('championId', champion_id, 'games', games, 'wins', wins, 'winrate', winrate, 'pickrate', pickrate, 'banrate', banrate, 'presence', presence, 'byRole', COALESCE(byRole, '{}'::jsonb)) ORDER BY games DESC)
  INTO champions_json FROM champ_with_roles;

  SELECT SUM(games)::int INTO total_games FROM (
    SELECT champion_id, COUNT(*) AS games FROM participants p INNER JOIN matches m ON m.id = p.match_id
    WHERE (p_rank_tier IS NULL OR p_rank_tier = '' OR p.rank_tier IN (SELECT UPPER(TRIM(unnest(string_to_array(p_rank_tier, ',')))))) AND (p_role IS NULL OR p_role = '' OR p.role = p_role)
