...
/** Parallel callers share one in-flight fetch per language. */
let loadChampionsInflight: Promise<void> | null = null
let loadChampionsInflightLang: string | null = null

/** Parallel detail loads for the same champion/lang share one fetch. */
const loadChampionDetailsInflight = new Map<string, Promise<Champion | null>>()
...
export const useChampionsStore = defineStore('champions', {
  ...
  actions: {
    ...
    async loadChampions(language: string = 'fr_FR') {
      ...
      if (loadChampionsInflight && loadChampionsInflightLang === language) {
        await loadChampionsInflight
        return
      }

      loadChampionsInflightLang = language
      loadChampionsInflight = (async () => {
        try {
          this.status = 'loading'
          this.error = null
          ...
          loadChampionsInflight = null
          loadChampionsInflightLang = null
        }
      })()
    ...
    async loadChampionDetails(
      ...
      const inflight = loadChampionDetailsInflight.get(cacheKey)
      if (inflight) {
        return inflight
      }
      ...
      const request = (async (): Promise<Champion | null> => {
        try {
          ...
          loadChampionDetailsInflight.delete(cacheKey)
        }
      })()

      loadChampionDetailsInflight.set(cacheKey, request)
