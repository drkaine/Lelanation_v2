<template>
  <div class="statistics flex min-h-screen min-w-0 flex-col overflow-x-hidden text-text">
    ...
    <div class="flex min-h-0 flex-1">
      <button
        v-if="showFiltersPanel && showDesktopFiltersTrigger"
        type="button"
      ...
      <div
        v-if="showFiltersPanel && filtersOpen && showFiltersBackdrop"
...
watch([filtersOpen, lockPageScrollForFilters, showFiltersPanel], () => {
  if (!import.meta.client) return
  const lock = lockPageScrollForFilters.value && filtersOpen.value && showFiltersPanel.value
...
async function bootstrapStatisticsPage(): Promise<void> {
  ...
  await Promise.all([versionPromise, loadVersionsWithMatches()])
...
// Ne pas await : lazy=true + await repousse onMounted hors du setup → erreur Nuxt 500 « 26 ».
useAsyncData(
...
onMounted(() => {
  ...
  if (!queryFirst(route.query.tab as string | string[] | null | undefined)) {
    activeTab.value = statisticsUiStore.ensureActiveTabVisible(
      statisticsUiStore.resolveDefaultMainTab
    )
  } else {
    activeTab.value = statisticsUiStore.ensureActiveTabVisible(activeTab.value as StatisticsMainTab)
  }
  applyStatisticsStateFromQuery()
